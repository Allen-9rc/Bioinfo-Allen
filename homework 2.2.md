...
### 2. ###
`grep -w IV 1.gtf | cut -f 3 | sort | uniq -c | sort -k 1`
```
    853 start_codon
    853 stop_codon
    886 gene
...
```
### 4. ###
`cat 1.gtf | awk -F"\t" '$1 == "XV" && $3 == "gene"{len = $5-$4+1; split($9,x,";"); name = x[1]; gsub("\"","",name); gsub("gene_id","",name); print name,len}' | sort -k2,2n | tail -n 5`
```
YOR142W-B 5269
YOR192C-B 5314
YOR343W-B 5314
